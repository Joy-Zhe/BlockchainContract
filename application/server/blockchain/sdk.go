...
//新建函数，完成区块链通道选择
// ChannelExecute 区块链交互
func ChannelExecute_new(fcn string, self_channelName string, args [][]byte) (channel.Response, error) {
	// 创建客户端，表明在通道的身份
	ctx := sdk.ChannelContext(self_channelName, fabsdk.WithUser(user))
	cli, err := channel.New(ctx)
	if err != nil {
		return channel.Response{}, err
	}
...
// ChannelQuery 区块链查询
func ChannelQuery_new(fcn string, self_channelName string, args [][]byte) (channel.Response, error) {
	// 创建客户端，表明在通道的身份
	ctx := sdk.ChannelContext(self_channelName, fabsdk.WithUser(user))
	cli, err := channel.New(ctx)
	if err != nil {
